...
import { animalBonus, animalImages, games, profilePriority, profiles, speciesList, type GameScenario, type Profile, type Species } from "../lib/game-data";
...
function fallbackImage(e: React.SyntheticEvent<HTMLImageElement>) { e.currentTarget.src = animalImages.Bovins; }
function photoFor(profile: Profile, selected: Species[]) { return animalImages[selected[0]] || (profile === "Veterinaire" ? animalImages.Chevaux : profile === "Proprietaire animaux" ? animalImages.Chiens : animalImages.Bovins); }
...
export default function App() {
  ...
  const [form, setForm] = useState({ first: "", last: "", phone: "", city: "", profile: "Eleveur" as Profile, animals: [] as Species[], identification: "Je ne sais pas", onsite: false });
  ...
  function tapAction(action: Action) {
    if (selected.includes(action.key)) return;
    navigator.vibrate?.(action.kind === "good" ? 25 : 10);
...
function MechanicView({ game, actions, selected, tapAction }: { game: GameScenario; actions: Action[]; selected: string[]; tapAction: (a: Action) => void }) {
  const firstGood = actions.find((a) => a.kind === "good");
  if (game.mechanic === "assign") return <Panel image={game.image}><div className="absolute inset-x-4 top-4 rounded bg-white/90 p-3 font-black">Touchez un animal puis son exploitation</div><AnimalDots game={game} /><div className="absolute inset-x-4 bottom-4 grid grid-cols-2 gap-2">{game.destinations?.map((d, i) => <button key={d} onClick={() => firstGood && tapAction(actions.find((a) => a.label.includes(d)) || (i === 3 ? actions[2] : actions[0]))} className="rounded bg-white/95 p-3 font-black shadow">{d}</button>)}</div></Panel>;
  if (game.mechanic === "map" || game.mechanic === "patient" || game.mechanic === "theft") return <Panel image={game.image}><div className="absolute inset-4 grid grid-cols-2 gap-3">{[...(game.facts || []), ...game.bad].slice(0, 8).map((f, i) => <button key={f} onClick={() => tapAction(actions.find((a) => a.label.toLowerCase().includes(f.toLowerCase().split(" ")[0])) || actions[i % actions.length])} className="rounded border bg-white/95 p-3 text-left font-black shadow">{f}</button>)}</div>{game.warning && <p className="absolute inset-x-4 bottom-4 rounded bg-amber/90 p-3 text-sm font-black text-white">{game.warning}</p>}</Panel>;
  if (game.mechanic === "market" || game.mechanic === "economy") return <Panel image={game.image}><div className="absolute inset-x-4 top-4 rounded bg-white/95 p-3 font-black">{game.facts?.[0]}</div><div className="absolute inset-x-4 bottom-4 grid grid-cols-3 gap-3">{game.animals.map((a, i) => <button key={a.id} onClick={() => tapAction(actions[i % actions.length])} className="rounded bg-white/95 p-3 text-left font-black shadow"><img onError={fallbackImage} src={a.image} alt={a.species} className="mb-2 h-20 w-full rounded object-cover" />{a.species}</button>)}</div></Panel>;
  if (game.mechanic === "veterinary" || game.mechanic === "memory") return <Panel image={game.image}><div className="absolute inset-x-4 top-4 rounded bg-white/95 p-3 font-black">Comparez les animaux similaires, puis confirmez par l'identifiant.</div><div className="absolute inset-x-4 bottom-4 grid grid-cols-3 gap-3">{game.animals.map((a, i) => <button key={a.id} onClick={() => tapAction(actions[i === 0 ? 0 : Math.min(i, actions.length - 1)])} className="rounded bg-white/95 p-2 font-black shadow"><img onError={fallbackImage} src={a.image} alt={a.species} className="h-24 w-full rounded object-cover" />{a.electronicId?.slice(-6)}</button>)}</div></Panel>;
  return <Panel image={game.image}><div className="absolute inset-x-4 top-4 rounded bg-white/95 p-3 font-black">Zone de scan virtuelle</div><AnimalDots game={game} /><button onClick={() => firstGood && tapAction(firstGood)} className="absolute rounded-full bg-tech p-6 font-black text-white shadow" style={{ left: game.animals[0]?.x + "%", top: game.animals[0]?.y + "%" }}>BIP</button><button onClick={() => tapAction(actions.find((a) => a.kind === "bad") || actions[0])} className="absolute bottom-5 right-5 rounded bg-white/95 p-4 font-black shadow">Aucune identification</button></Panel>;
}

function Panel({ image, children }: { image: string; children: React.ReactNode }) { return <div className="relative min-h-[520px] overflow-hidden rounded border bg-white shadow"><img onError={fallbackImage} src={image} alt="Animal" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-black/10" />{children}</div>; }
...
function PhotoStrip() { return <div className="mb-5 grid grid-cols-4 gap-3 grid2">{speciesList.slice(4).map((s) => <img key={s} onError={fallbackImage} src={animalImages[s]} alt={s} className="h-36 w-full rounded object-cover shadow" />)}</div>; }
...
function GameAdmin({ configs, setConfigs }: { configs: GameConfig[]; setConfigs: (c: GameConfig[]) => void }) { function patch(id: string, update: Partial<GameConfig>) { setConfigs(configs.map((c) => c.id === id ? { ...c, ...update } : c)); } return <div className="grid grid-cols-3 gap-4 grid2">{games.map((g) => { const c = configs.find((x) => x.id === g.id) || baseConfig[0]; return <article key={g.id} className="overflow-hidden rounded border bg-white shadow"><img onError={fallbackImage} src={g.image} alt={g.species.join(", ")} className="h-40 w-full object-cover" /><div className="p-4"><h2 className="text-xl font-black">{g.title}</h2><p className="mt-2 text-sm text-ink/70">{g.mission}</p><p className="mt-2 text-xs font-bold text-tech">Profils : {g.profiles.join(", ")}</p><label className="mt-3 flex items-center gap-3 font-bold"><input type="checkbox" checked={c.active} onChange={(e) => patch(g.id, { active: e.target.checked })} /> Actif</label><div className="mt-3 grid grid-cols-3 gap-2"><Num label="Duree" value={c.duration} set={(v) => patch(g.id, { duration: v })} /><Num label="Freq." value={c.frequency} set={(v) => patch(g.id, { frequency: v })} /><Num label="Manches" value={c.rounds} set={(v) => patch(g.id, { rounds: v })} /></div><select value={c.difficulty} onChange={(e) => patch(g.id, { difficulty: e.target.value as GameConfig["difficulty"] })} className="tap mt-3 w-full rounded border px-3"><option>facile</option><option>normal</option><option>difficile</option></select></div></article>; })}</div>; }
