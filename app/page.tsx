...
type Lead = { id: string; first: string; last: string; phone: string; city: string; animalCount: number; profile: Profile; animals: Species[]; identification: string; onsite: boolean; game: string; concreteResult: string; score: number; percentage: number; durationSeconds: number; gift: string; metadata: Record<string, unknown>; createdAt: string; sync: "Synchronise" | "En attente" };
type FormState = { first: string; last: string; phone: string; city: string; animalCount: number; profile: Profile; animals: Species[]; identification: string; onsite: boolean };
...
const emptyForm: FormState = { first: "", last: "", phone: "", city: "", animalCount: 0, profile: "Eleveur", animals: [], identification: "Je ne sais pas", onsite: false };
...
export default function App() {
  ...
  const declaredAnimals = leads.reduce((total, lead) => total + (lead.animalCount || 0), 0);
  ...
  function exportCsv() {
    const header = "Nom,Telephone,Ville,Profil,Especes,Nombre animaux,Jeu auto,Resultat,Score,Lot,Duree,Date\n";
    const rows = leads.map((l) => [l.first + " " + l.last, l.phone, l.city, l.profile, l.animals.join(" | "), String(l.animalCount || 0), l.game, l.concreteResult, l.percentage + "%", l.gift, l.durationSeconds + "s", l.createdAt].map((x) => `"${x.replaceAll('"', '""')}"`).join(","));
...
function LeadTable({ leads }: { leads: Lead[] }) { return <div className="overflow-auto rounded border bg-white"><table className="w-full min-w-[1180px] text-left"><thead className="bg-field"><tr>{["Nom", "Telephone", "Profil", "Especes", "Nombre animaux", "Jeu auto", "Resultat", "Score", "Lot", "Duree"].map((h) => <th className="p-3" key={h}>{h}</th>)}</tr></thead><tbody>{leads.map((l) => <tr className="border-t" key={l.id}><td className="p-3 font-bold">{l.first} {l.last}</td><td className="p-3">{l.phone}</td><td className="p-3">{l.profile}</td><td className="p-3">{l.animals.join(", ")}</td><td className="p-3 font-black text-tech">{l.animalCount || 0}</td><td className="p-3">{l.game}</td><td className="p-3">{l.concreteResult}</td><td className="p-3">{l.percentage}%</td><td className="p-3">{l.gift}</td><td className="p-3">{l.durationSeconds}s</td></tr>)}</tbody></table>{!leads.length && <p className="p-6 text-center font-bold text-ink/60">Aucun participant pour le moment.</p>}</div>; }
